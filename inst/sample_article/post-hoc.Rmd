...
```{r}
rwt <- sansSouci::rowWelchTests(dat, colnames(dat))
class(rwt) <- "list"
...
## From FDR control to post hoc inference
...
### R packages used in this vignette
...
### 0 - Gene sets of interest

TODO

- pre-defined genes sets (mapped to gene identifiers in the data set...)
- table with bound on the FDP in each gene set

Take home message: validity of the bounds even if one adds more gene sets.

### 1 - Confidence envelopes

```{r}
stat <- dex[["statistic"]]
stat <- qnorm(dex[["p.value"]])
m <- length(stat)
...
plot(1:K, 1:K - ub[1:K], t = 's', col = "purple",
     ylab = "Lower bound on the number of true positives in selection")
...
output$bound <- renderText({
  ...
  msg <- ""
  if (is.null(d)) {
    msg <- sprintf("Default selection: BH procedure at level %s for FDR control\n", alpha)
    mm <- which(datly[["adjp.BH"]] <= alpha)
  } else {
    mm <- match(d$key, datly[["id"]])
  }
